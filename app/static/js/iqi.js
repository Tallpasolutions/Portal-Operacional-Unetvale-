...
(function () {
  ...
  let modo = "meta"; // meta = dentro da meta (padrão) | fora = acima da meta (invertido)
  ...
  // Fora da meta (visão invertida): SÓ o percentual conta — sem mínimo de OSs
  // (basta ter OS no mês). Reincidência = meses consecutivos acima da meta.
  const estourou = (reg) => reg[0] > 0 && reg[2] >= META;
  ...
  function ranking(idx) {
    ...
    return DATA.tecnicos.filter((t) => crit(t.m[idx])).map((t) => ({
  ...
  function aplicar(data) {
    ...
    // Todos os meses entram no seletor (acompanham o andamento do mês corrente);
    // os ainda em auditoria levam a tag "(Parcial)". Padrão = último homologado.
    const mesFechado = (m) => {
    ...
    sel.innerHTML = data.meses.map((m, i) =>
      `<option value="${i}">${m}${mesFechado(m) ? "" : " (Parcial)"}</option>`).join("");
    let def = data.meses.length - 1;
    for (let i = data.meses.length - 1; i >= 0; i--) if (mesFechado(data.meses[i])) { def = i; break; }
  ...
  function desenhar() {
    ...
    if (qtdLabel) qtdLabel.textContent = fora ? "Fora da meta" : "Equipes na meta";
    const starTxt = document.getElementById("starTxt");
    if (starTxt) starTxt.textContent = fora ? "= meses consecutivos fora da meta" : "= meses consecutivos na meta";
    const tabTitulo = document.getElementById("tabTitulo");
    if (tabTitulo) tabTitulo.textContent = fora ? "Técnicos fora da meta" : "Técnicos dentro da meta";
    document.getElementById("sub").textContent = fora
      ? `${IND} = OSs com atendimento/reincidência em até ${DATA.dias} dias após a ${DATA.evento} ÷ total de OSs. ` +
        `Visão FORA da meta: considera apenas o percentual (sem mínimo de OSs).`
      : `${IND} = OSs com atendimento/reincidência em até ${DATA.dias} dias após a ${DATA.evento} ÷ total de OSs. ` +
    ...
    const corPad = fora ? "#e63757" : "#2c7be5";
    const corSel = fora ? "#8f1e33" : "#1f5fc0";
    const corStar = fora ? "#8f1e33" : "#e5a000";
    const cores = chartData.map((d) => (selecionado && d.nome === selecionado) ? corSel : corPad);
    ...
    chart = new Chart(document.getElementById("g-iqi"), {
      ...
      data: {
        ...
        datasets: [
          ...
          // na visão fora da meta (barras vermelhas) a linha de meta vai p/ grafite
          { label: "Meta", type: "line", data: chartData.map(() => META), borderColor: fora ? "#344050" : "#e63757",
  ...
  function exportarImagem() {
    ...
    const fora = modo === "fora";
    ctx.fillText(`${IND} — ${mesTxt} — Meta < ${fmtMeta(META)}${fora ? " — FORA DA META" : ""}`, 16, 26);
    ctx.fillStyle = "#5e6e82"; ctx.font = "12px -apple-system, Segoe UI, Roboto, Arial";
    ctx.fillText(fora
      ? `Técnicos fora da meta: ${document.getElementById("qtd").textContent}  ·  ★ = meses consecutivos fora da meta (só percentual, sem mínimo de OSs)`
      : `Equipes na meta: ${document.getElementById("qtd").textContent}  ·  ★ = meses consecutivos batendo a meta`, 16, 45);
    ctx.drawImage(chart.canvas, 0, headH, cssW, cssH);
    const a = document.createElement("a");
    a.href = out.toDataURL("image/png");
    a.download = `${IND}_${mesTxt.replace("/", "-")}${fora ? "_fora_da_meta" : ""}.png`;
    a.click();
  }
  ...
  function renderTabela(base) {
    ...
    const pctCls = modo === "fora" ? 'class="num" style="color:var(--danger);font-weight:700;"' : 'class="num verde"';
    document.querySelector("#tab-iqi tbody").innerHTML = data.map((d) => {
      const extra = d.stars - 1;
      return `<tr data-nome="${d.nome}" style="cursor:pointer;">
        <td>${d.nome}</td><td ${pctCls}>${fmt(d.iqi)}</td><td class="num">${d.os}</td>
  ...
  // Alterna a visão dentro/fora da meta (gráfico invertido)
  document.getElementById("btn-modo-iqi").addEventListener("click", () => {
    modo = modo === "meta" ? "fora" : "meta";
    const b = document.getElementById("btn-modo-iqi");
    b.classList.toggle("on", modo === "fora");
    b.querySelector(".txt").textContent = modo === "fora" ? "Ver dentro da meta" : "Ver fora da meta";
    selecionado = null;
    desenhar();
  });
